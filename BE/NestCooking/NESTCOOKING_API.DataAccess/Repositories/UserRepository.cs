...
namespace NESTCOOKING_API.DataAccess.Repositories
{
    public class UserRepository : Repository<User>, IUserRepository
    {
        ...
        public async Task IncreaseUserBalanceAsync(string userId, double amount)
        {
            var userFromDb = await this.GetAsync(u => u.Id == userId);

            if (userFromDb != null)
            {
                if (_context.Entry(userFromDb).State == EntityState.Detached)
                {
                    _context.Attach(userFromDb);
                }
